...
def test_dbms_config_generator():
    ...
    knob_spec_path = os.path.join(
        proj_dir, "cybernetics/knobs/postgres_12.17_selected_knobs.json"
    )
    assert os.path.exists(knob_spec_path)
    random_seed = 12345

    dbms_config_space_generator = KnobSpaceGenerator(
        knob_spec_path, random_seed
    )
    ...
    if "work_mem" in dbms_config_space:
        ...
        assert (
            dbms_config_sample["work_mem"] >= min_val
            and dbms_config_sample["work_mem"] <= max_val
        )
