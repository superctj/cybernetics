...
def load_json_files(directory, suffix):
    """加载指定目录中的 JSON 文件，按照文件名中的时间戳排序，并忽略空文件或无效 JSON 文件。"""
    json_files = []  # 用于存储合法的文件数据
    files_with_dates = []  # 用于记录文件名及其时间戳
    ...
    for filename, _ in files_with_dates:
        ...
        try:
            ...
            # 尝试加载 JSON 数据
            with open(file_path, 'r') as file:
                data = json.load(file)  # 如果文件内容无效，这里会抛出错误
                json_files.append((filename, data))
        except json.JSONDecodeError as e:
            print(f"文件 {filename} 是无效的 JSON: {e}")
        except Exception as e:
...
def plot_throughput(throughput_data, best_throughput):
    ...
    output_dir = '/home/boyuann/cybernetics/exps/benchbase_tpcc/postgres/bo_gp'
...
if __name__ == "__main__":
    csv_directory = "/home/boyuann/cybernetics/exps/benchbase_tpcc/postgres/bo_gp"
