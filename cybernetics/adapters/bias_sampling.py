...
https://github.com/uw-mad-dash/llamatune/blob/main/adapters/bias_sampling.py
"""
...
def special_value_scaler(hp, value):
    ...
    return hp._inverse_transform(
        hp._transform_scalar(
            (value - hp._special_value_prob) / (1.0 - hp._special_value_prob)
        )
    )


# Used for when you do not want biased sampling
class UniformIntegerHyperparameterWithSpecialValue(
    ...
    def __init__(
        ...
        *args,
        ...
        special_value_prob: Optional[float],
        **kwargs
    ):
        ...
        assert (
            special_value_prob > 0 and special_value_prob < 1
        ), "Probability for special value should be in (0, 1)"
        assert (
            special_value == self.lower
    ...
    def _sample(
        self, rs: np.random.RandomState, size: Optional[int] = None
    ...
    def __repr__(self) -> str:
        ...
        repr_str.write(
            "%s, Type: UniformIntegerWithSpecialValues, Range: [%s, %s], Default: %s"
            % (
                self.name,
                repr(self.lower),
                repr(self.upper),
...
class PostgresBiasSampling:
    def __init__(
        self, adaptee: CS.ConfigurationSpace, seed: int, bias_prob_sv: float
    ):
        ...
        assert (
            bias_prob_sv > 0 and bias_prob_sv < 1
