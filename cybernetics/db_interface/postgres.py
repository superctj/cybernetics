...
class PostgresWrapper:
    ...
    def _start_postgres(self) -> bool:
        ...
        count = 0
        start_success = True
        logger.info("Wait for Postgres connection...")
        ...
        while True:
            ...
            if count > 600:
                start_success = False
                logger.info("Can not connect to Postgres.")
                ...
                logger.info("Kill all Postgres processes.")
                break

        logger.info(f"Postgres is up! Waited {count} seconds for connection.")
    ...
    def _kill_postgres(self) -> None:
        ...
        try:
            ...
            if ret_code == 0:
                logger.info("Close Postgres successfully.")
        except subprocess.TimeoutExpired:
            logger.info("Force close Postgres.")
            os.system(force_kill_cmd1)
            os.system(force_kill_cmd2)
            logger.info("Postgres is shut down")
    ...
    def set_knob_value(self, db_conn, k, v) -> bool:
        ...
        try:
            ...
            logger.info(f"Failed to execute {sql}")
    ...
    def apply_knobs_online(self, knobs):
        ...
        pg_client.close_db()
        logger.info(f"[{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}] Knobs applied online!")
    ...
    def apply_knobs_offline(self, knobs: dict) -> bool:
        ...
        try:
            logger.info(f"Sleep for {RESTART_WAIT_TIME} seconds after restarting Postgres...")
